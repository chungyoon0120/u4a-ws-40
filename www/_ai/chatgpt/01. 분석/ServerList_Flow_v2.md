...
- 화면 생성 로직과 데이터 처리 로직 분리
- Electron/Node 의존 로직과 DOM UI 로직 분리
...
`css/` 기존 폴더는 초기 전환 중 호환용으로 유지하고, 신규 CSS는 `assets/css/`로 분리한다. 전환이 완료되면 기존 `css/server.css`, `css/serverInfo.css`는 제거하거나 신규 CSS에 흡수한다.

---

## 3. ServerList.html 역할

`ServerList.html`은 UI5 bootstrap을 제거하고, 순수 HTML Shell과 JS 진입점만 담당한다.
...
<head>
  ...
  <title>U4A Workspace Logon Pad</title>
...
Electron 기능은 유지하므로 SAPGUI 감지, Registry, 파일 저장, Named Pipe는 기존 구조를 재사용하거나 service 계층으로 감싸서 이전하면 된다. 실제 제거 대상은 UI5 렌더링 계층이며, 데이터 처리와 Electron 연동 로직은 분리 후 재활용하는 방향이 가장 안전하다.
