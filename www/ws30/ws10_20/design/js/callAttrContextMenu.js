(function(){
    ...
    oAPP.fn.callAttrContextMenu = function(){
         ...
         * @author  pes
         * @description
         * UI의 이벤트 설정건에 대한 단축키 메뉴
         * 3.5.7-4 패치 버전에서 해당 메뉴를 활성화 처리함.
         */
        if(oAPP.common.checkWLOList("C", "UHAK901289") === true){
            //805   단축키 등록
            let _txt = parent.WSUTIL.getWsMsgClsTxt("", "ZMSG_WS_COMMON_001", "805");    

            oMenu1.addItem(new sap.m.MenuItem({
                key:"M05", 
                icon:"sap-icon://keyboard-and-mouse", 
                text:_txt, 
                tooltip:_txt, 
                visible:"{/atmenu/vis05}", 
                startsSection:true
            }));
        }
         ...
         * @author  pes
         * @description
         * UI ATTRIBUTE의 개인화 팝업 CONTEXT MENU 등록 처리.
         */    
        //627	UI Attribute Personalization
        let _txt = parent.WSUTIL.getWsMsgClsTxt("", "ZMSG_WS_COMMON_001", "627");        
    ...
    oAPP.fn.attrCtxtMenuItemPress = async function(oUi, key){
        ...
        switch(key){
            ...
            case "M05":
                 ...
                 * @author  pes
                 * @description
                 * UI의 이벤트 설정건에 대한 단축키 등록처리 기능 호출.
                 */
                ...
                let _shortcutPath  = parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, "attrContextMenu", "eventShortcutReg.js");

                let _oShortcut = await import(_shortcutPath);

                _oShortcut.default(_sAttr);
             ...
             * @author  pes
             * @description
             * UI ATTRIBUTE의 개인화 팝업 CONTEXT MENU 선택에 따른 로직 처리.
             */ 
            case "M06":
                //UI ATTRIBUTE의 개인화 팝업 호출.
                _callP13nAttrPopup(oUi);
    ...
    oAPP.fn.attrSetContextMenu = function(oMenu, is_attr, sKey){
        ...
        ls_menu.vis05 = false;  //단축키 등록 처리.
        ls_menu.vis06 = false;  //개인화 팝업 호출 처리.
        ...
        switch (sKey) {
            ...
            case "AT02":
                ...
                if(is_attr.UIATY === "1" && is_attr.ISBND === ""){
                    ...
                    //입력 가능한 프로퍼티 항목에 대해 단축키 등록 메뉴 활성화 처리.
                    if(is_attr.edit === true){
                        //개인화 팝업 메뉴 활성화 처리.
                        ls_menu.vis06 = true;
     ...
     * @author  pes
     * @description
     * UI ATTRIBUTE의 개인화 팝업 호출 처리.
     */    
    async function _callP13nAttrPopup(oUi){
        ...
        let _shortcutPath  = parent.PATH.join(oAPP.oDesign.pathInfo.designRootPath, "attrPresetPopup", "settings", "index.js");

        let {attrPresetPopup} = await import(_shortcutPath);

        attrPresetPopup({sAttr: _sAttr});
