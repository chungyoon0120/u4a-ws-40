...
oAPP = {
    ...
    onRecordActive: () => {
        ...
        const mainWIN = oAPP.remote.getCurrentWindow();

        var 
            sBound = mainWIN.getBounds();
            sBound = JSON.parse(JSON.stringify(sBound));

        const X = sBound.x + screen.availWidth / 2;
        ...
        let oWIN = new oAPP.remote.BrowserWindow(oBrowserOptions);
            oWIN.setMenuBarVisibility(false);
            oWIN.setAlwaysOnTop(true, "screen-saver");
        ...
        const sUrlPath = parent.PATH.join(__dirname, "RecordActive.html");

        // URL에 QueryString 파라미터를 적용한다.
        const sLoadUrl = oAPP.WSUTIL.QueryString.build(sUrlPath, oQueryParams);

        oWIN.loadURL(sLoadUrl);
    ...
    onControllerOpen: () => {
        ...
        var oWIN = new oAPP.remote.BrowserWindow(oBrowserOptions);
            oWIN.setMenuBarVisibility(false);
            oWIN.setAlwaysOnTop(true, "screen-saver");
        ...
        const sUrlPath = parent.PATH.join(__dirname, "controller.html");
    ...
    onInitData: () => {
        ...
        // UserData 폴더 경로
        let sUserDataPath = oAPP.remote.app.getPath("userData");

        //동영상 저장 폴더 경로 설정 
        oAPP.saveDirectory = oAPP.path.join(sUserDataPath, "video");
